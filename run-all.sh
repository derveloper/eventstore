...
vertx run eventstore.writer.WriteEventsVerticle \
    -cp event-writer/target/*fat* -cluster -ha -cluster-host 192.168.178.32 --instances 1 &

vertx run eventstore.cache.EventCacheVerticle \
    -cp event-cache/target/*fat* -cluster -ha -cluster-host 192.168.178.32  --instances 1 &

vertx run eventstore.reader.ReadEventsVerticle \
    -cp event-reader/target/*fat* -cluster -ha -cluster-host 192.168.178.32  --instances 1 &

vertx run eventstore.boundary.PushApiVerticle \
    -cp push-api/target/*fat* -cluster -ha -cluster-host 192.168.178.32  --instances 1 &
...
vertx run eventstore.persistence.RethinkDBEventPersistenceVerticle \
    ...
    -cluster -ha -cluster-host 192.168.178.32  --instances 1 &
